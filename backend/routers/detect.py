"""
Synchronous detection endpoint for frontend.
Uses Ollama VLM for image analysis. When project_id is provided, design spec PDFs
are read from MinIO and their text is passed to the VLM so inspection follows the spec.
When Ollama is unavailable, returns a mock result so the UI still shows a result.
"""
